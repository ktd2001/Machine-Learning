...
#Training and Testing - Random Sample uses sample())
samples <- sample(1:nrow(ProfData), size=0.8*nrow(ProfData))

#Gave names to the value sample so it does not overlap with sample()
...
errors <- function(Y, y.hat_data){ #Even though we need to use y data and y.hat data, 
  ...
  r_squared <- 1 - (RSS/TSS) #Equation for r^2 = 1 minus the RSS value divided by the TSS value
...
#In conclusion, both R^2 values from the regression from scratch and also using the built in functions with R are close to 
#1 which means they values are associated, but are also very close to each other. 
#It is interesting to see how different the RSS/TSS values are but the R^2 value is still extremely close.
